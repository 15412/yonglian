...
class GeneralCategoryModel extends Model{
     ...
     * @author 麦当苗儿 <zuojiazi@vip.qq.com>
     */
    public function getTree($id = 0, $field = true, $type=0){
        /* 获取当前分类信息 */
        if($id){
            $info = $this->info($id);
            $id   = $info['id'];
        ...
        $map['type'] = $type?array('eq', $type):array('gt', 0);
        //$list = $this->field($field)->where($map)->order('sort')->select();
        $list = $this->field($field)->where($map)->order('sort asc,create_time')->select();
    ...
    public function getSameLevel($id, $field = true){
        ...
        return $this->field($field)->where($map)->order('sort desc,create_time')->select();
    }
